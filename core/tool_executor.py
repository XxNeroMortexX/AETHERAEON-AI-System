"""
========================================================
AETHERAEON — TOOL EXECUTION LAYER (SYSTEM ACTION ENGINE)
========================================================

FILE PURPOSE:
This file is the ACTION ENGINE of the AI system.

It is responsible for executing real-world operations
requested by the AI Orchestrator AFTER reasoning is complete.

========================================================
SYSTEM ROLE:
"Hands Layer" of the architecture.

It does NOT think.
It does NOT decide.
It ONLY executes structured tool requests.

========================================================
RESPONSIBILITIES:
(tool_executor.py)

- Execute tool calls generated by ai_orchestrator.py
- Run external APIs (web search, weather, etc.)
- Execute automation workflows (playbooks, scripts, n8n flows)
- Handle file system operations (safe sandboxed only)
- Process structured tool payloads
- Return clean execution results to request_router.py
...
- Perform AI reasoning or interpretation
- Make decisions about which tool to use
- Access memory system directly (memory_database.py handles this)
- Perform model selection (model_registry.py handles this)
- Handle user intent classification
- Modify UI state or frontend logic

It ONLY executes instructions it receives.

========================================================
TOOL EXECUTION FLOW:
(tool_executor.py internal pipeline)

ai_orchestrator.py
    ↓
structured tool_request generated
    ↓
request_router.py
    ↓
tool_executor.py   ← THIS FILE
    ↓
validate tool payload
    ↓
select tool from tool_registry
    ↓
execute tool via external_toolkit.py or internal handlers
    ↓
capture result / errors / metadata
    ↓
return structured output
    ↓
request_router.py → api_gateway.py → UI

========================================================
SUPPORTED TOOL TYPES:

- web_search
- api_call (REST / external services)
- file_read / file_write
- system_command (restricted sandbox)
- playbook_execution (automation workflows)
- memory_tools (via memory_interface bridge ONLY)
- custom registered tools (tool_registry.py)

========================================================
OUTPUT CONTRACT:

tool_executor MUST return:
